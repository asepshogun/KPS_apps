...
<script type="text/javascript">
    document.getElementById("titleHeader").innerHTML = '<font size="3"><b> | Delivery Order Replacement V 3.0</b></font>';
</script>
<style type="text/css">
			...
			<div class="form-group">
				<div class="box-body">	
			        ...
			        <div class="col-md-3">
                        ...
                        <a class="btn bg-red" onClick="setup_data_dor();" href="#"><b><i class="fa fa-pencil"></i> &nbsp;Add New Dor</b></a>
               	 	</div>  
	           	</div>
			</div>
        </form>
...
<script type="text/javascript">
	function addContent(settings) {
        ...
        if(loading == false && end_record == false){
            ...
            $.ajax({
                method: "POST", 
                type  : 'ajax',
                url   : '<?php echo site_url() ?>/Dor_v3/LoadData',
                ...
                success : function(data){
                	for(i=0; i<data.length; i++){
                		...
                		settings.htmldata += '<tr id="rowd_'+offsetN0+'">'+
                                ...
                                '<td>'+new Date(data[i].KPS_DO_RETUR_DATE).format("dd-mm-yyyy")+'</td>'+
                                '<td>'+data[i].KPS_DO_RETUR_NO+'</td>'+
                                '<td>'+data[i].OUTGOING_RETUR_PRODUCT_ID_DO+'</td>'+
                                '<td><center><a class="btn bg-orange" onClick="update_data('+data[i].KPS_DELIVERY_ORDER_RETUR_ID+');" href="#"><b><i class="fa fa-gears"></i> &nbsp;Action</b></a></center></td>';
                	}
                	htmlDataFooter="";
            ...
            headerTable     :'<table id="data2" class="table table-bordered table-hover table-striped "'+
                                ...
                                '<thead style="background-color : #337ab7; color:white;">'+
                                    '<tr style="white-space: nowrap;">'+
                                        ...
                                        '<th><center>DO_RETUR_DATE</center></th>'+
                                        '<th><center>DO_RETUR_NO</center></th>'+
                                        '<th><center>OUTGOING RETUR PRODUCT NO</center></th>'+
	...
	function setup_data_dor(){
		// console.log("ada");
		$.ajax({
		    url   : '<?php echo site_url() ?>/Dor_v3/LoadDataForSetup',
		    success : function(data){
	...
	function update_data(id){
		// console.log(id);
		$.ajax({
		    url   	: '<?php echo site_url() ?>/Dor_v3/LoadDataForUpdate',
		    data 	: {'id':id},
		    method	: "POST",
		    success : function(data){
		    	console.log(data);
