...
<script type="text/javascript">
	function addContent(settings) {
        ...
        if(loading == false && end_record == false){
            ...
            $.ajax({
                method: "POST", 
                type  : 'ajax',
                url   : '<?php echo site_url() ?>/retur_product_outgoing_v3/LoadData',
                ...
                success : function(data){
                	for(i=0; i<data.length; i++){
                		offsetN0++;
                		if (data[i].approve_by==null || data[i].approve_by=='null') {
                			data[i].approve_by="-";
                		}
                		if (data[i].check_by==null || data[i].check_by=='null') {
                			data[i].check_by="-";
                		}
                		if (data[i].KPS_DO_RETUR_NO==null || data[i].KPS_DO_RETUR_NO=='null') {
                			data[i].KPS_DO_RETUR_NO="-";
                		}
                		settings.htmldata += '<tr id="rowd_'+offsetN0+'" style="white-space: nowrap;">'+
                                ...
                                '<td><center><a class="btn bg-blue" onClick="detail('+data[i].OUTGOING_RETUR_PRODUCT_ID+');" href="#"><b><span class="glyphicon glyphicon-log-in"></span>&nbsp;Detail</b></a></center></td>'+
                                ...
                                '<td>'+data[i].KPS_OUTGOING_RETUR_REVISI_NOMOR+'</td>'+
                                '<td>'+data[i].VEHICLE_NO+'</td>'+
                                '<td>'+data[i].VEHICLE_NAME+'</td>'+
                                '<td>'+data[i].driver+'</td>'+
                                '<td>'+data[i].OUTGOING_RETUR_ASISTEN_DRIVER+'</td>'+
                                '<td>'+data[i].approve_by+'</td>'+
                                '<td>'+data[i].check_by+'</td>'+
                                '<td>'+data[i].MADE_BY+'</td>'+
                                '<td>'+data[i].KPS_DO_RETUR_NO+'</td>';
                	}
                	htmlDataFooter="";
            ...
            headerTable     :'<table id="data2" class="table table-bordered table-hover table-striped "'+
                                ...
                                '<thead style="background-color : #337ab7; color:white;">'+
                                    '<tr style="white-space: nowrap;">'+
                                        ...
                                        '<th><center>Retur No</center></th>'+
                                        '<th><center>Outgoing No</center></th>'+
                                        '<th><center>Rev No</center></th>'+
                                        ...
                                        '<th><center>Approve By</center></th>'+
                                        '<th><center>Check By</center></th>'+
                                        '<th><center>Made By</center></th>'+
                                        '<th><center>Delivery Order</center></th>',
	...
	function setup_data_orp(){
		// console.log("ada");
		$.ajax({
		    url   : '<?php echo site_url() ?>/retur_product_outgoing_v3/LoadDataForSetup',
		    success : function(data){
	...
	function update_data(id){
		// console.log(id);
		$.ajax({
		    url   	: '<?php echo site_url() ?>/retur_product_outgoing_v3/LoadDataForUpdate',
		    data 	: {'id':id},
		    method	: "POST",
		    success : function(data){
		    	console.log(data);
	...
	function get_dataForRetur() {
		$.ajax({
		    ...
		    url   : '<?php  echo site_url() ?>/retur_product_outgoing_v3/get_dataForRetur/',
		    dataType : 'json',
		    async	:true,
		    success : function(data){
		    	console.log(data);
